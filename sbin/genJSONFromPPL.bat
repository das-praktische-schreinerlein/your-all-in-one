...
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_JSONN% -m %PROJNAME% %SRC_OPTIONS%  %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%.ppl
echo "create JSON from PPL: %PROG_JSONN% -m %PROJNAME% %SRC_OPTIONS%  %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%.ppl > %MMPATH%\%SRCFILE%.tmp"
rem echo %CMD%
