...
rem     import to JPA from an existing wiki-file
rem <h4>Syntax:</h4>
rem     PROG projektpath filename_without_extension projectname
rem <h4>Example:</h4>
rem     cd D:\public_projects\yaio\yaio
rem     sbin\importJPAFromWiki.bat src\test\testproject\ test Testprojekt
rem 
rem @package de.yaio
...
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_JPAN% %CFG% -m %PROJNAME% %SRC_OPTIONS% %IMPORT_OPTIONS_JPA% %MMPATH%%SRCFILE%.wiki
echo "import JPA from Wiki: %PROG_JPAN% %CFG% -m %PROJNAME% %SRC_OPTIONS% %IMPORT_OPTIONS_JPA% %MMPATH%%SRCFILE%.wiki > %MMPATH%\%SRCFILE%.tmp"
rem echo %CMD%
