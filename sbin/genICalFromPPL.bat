...
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_ICalN% -m %PROJNAME% %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%-events.ppl
echo "create ICal from PPL: %PROG_ICalN% -m %PROJNAME% %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%-events.ppl > %MMPATH%\%SRCFILE%-events.ics"
%CMD% > %MMPATH%\%SRCFILE%-events.ics

rem create ToDo-ICal from PPL
grep -v EVENT_ %MMPATH%%SRCFILE%.ppl > %MMPATH%%SRCFILE%-tasks.ppl
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_ICalN% -m %PROJNAME% %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%-tasks.ppl
echo "create ICal from PPL: %PROG_ICalN% -m %PROJNAME% %OUTPUT_OPTIONS% %MMPATH%%SRCFILE%-tasks.ppl > %MMPATH%\%SRCFILE%-tasks.ics"
