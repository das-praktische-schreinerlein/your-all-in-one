...
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_JPAN% -m %PROJNAME% %SRC_OPTIONS% %IMPORT_OPTIONS_JPA% %MMPATH%%SRCFILE%.ppl
echo "import JPA from PPL: %PROG_JPAN% -m %PROJNAME% %SRC_OPTIONS% %IMPORT_OPTIONS_JPA% %MMPATH%%SRCFILE%.ppl > %MMPATH%\%SRCFILE%.tmp"
rem echo %CMD%
