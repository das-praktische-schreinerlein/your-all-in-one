...
set CMD=java %JAVAOPTIONS% -cp %CP% %PROG_EX% -m %PROJNAME% %OUTPUT_OPTIONS%  --mergeexcelplanunggantsheets %MMPATH%%SRCFILE%.ppl %MMPATH%%SRCFILE%.xls
echo "create Excel from PPL: %PROG_EX% -m %PROJNAME% %OUTPUT_OPTIONS%  --mergeexcelplanunggantsheets %MMPATH%%SRCFILE%.ppl %MMPATH%%SRCFILE%.xls"
rem echo %CMD%
%CMD%
