    ...
    me.yaioDoUpdateNode = function(node, url, json) {
        ...
        $.ajax({
                ...
                if (response.state == "OK") {
                    ...
                    if (response.parentIdHierarchy && response.parentIdHierarchy.length > 0) {
                        ...
                        tree.reload().done(function(){
                            ...
                            yaioAppBase.get('YaioExplorerAction').openNodeHierarchy("#tree", response.parentIdHierarchy);
                        });
                    } else {
                        yaioAppBase.get('YaioBase').logError("got no hierarchy for:" + node.key 
                    ...
                    if (response.violations) {
                        ...
                        for (var idx in response.violations) {
                            ...
                            yaioAppBase.get('YaioBase').logError("violations while save node:" + node.key 
            ...
            error : function(jqXHR, textStatus, errorThrown) {
                ...
                yaioAppBase.get('YaioBase').logError("The following error occured: " + textStatus + " " + errorThrown, true);
                yaioAppBase.get('YaioBase').logError("cant save node:" + node.key + " error:" + textStatus)
            },
            complete : function() {
        ...
        $.ajax({
            ...
            success : function(response, textStatus, jqXhr) {
                ...
                if (response.state == "OK") {
                    ...
                    if (response.node) {
                        var $nodeDataBlock = yaioAppBase.get('YaioNodeDataRender').renderDataBlock(response.node, fancynode);
                        ...
                        if (! treeNode) {
                            yaioAppBase.get('YaioBase').logError("error yaioLoadSymLinkData: cant load node - " + msg, false);
                            return null;
                        }
                        ...
                        yaioAppBase.get('YaioBase').logError("ERROR got no " + msg, true);
                    }
                } else {
                    yaioAppBase.get('YaioBase').logError("ERROR cant load  " + msg + " error:" + response.stateMsg, true);
                }
            },
            error : function(jqXHR, textStatus, errorThrown) {
                // log the error to the console
                yaioAppBase.get('YaioBase').logError("ERROR  " + msg + " The following error occured: " + textStatus + " " + errorThrown, false);
                yaioAppBase.get('YaioBase').logError("cant load " + msg + " error:" + textStatus, true)
            },
            complete : function() {
        ...
        $.ajax({
            ...
            success : function(response, textStatus, jqXhr) {
                ...
                if (response.state == "OK") {
                    ...
                    yaioAppBase.get('YaioBase').logError("cant remove node:" + node.key + " error:" + response.stateMsg, false);
                    // check for violations
                    if (response.violations) {
                        // iterate violations
                        for (var idx in response.violations) {
                            var violation = response.violations[idx];
                            yaioAppBase.get('YaioBase').logError("violations while remove node:" + node.key 
                                    + " field:" + violation.path + " message:" + violation.message, false);
            ...
            error : function(jqXHR, textStatus, errorThrown) {
                // log the error to the console
                yaioAppBase.get('YaioBase').logError("The following error occured: " + textStatus + " " + errorThrown, false);
                yaioAppBase.get('YaioBase').logError("cant remove node:" + node.key + " error:" + textStatus, true)
            },
            complete : function() {
